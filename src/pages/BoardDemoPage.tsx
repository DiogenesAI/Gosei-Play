...
const BoardDemoPage: React.FC = () => {
  ...
  const handlePlaceStone = (position: Position) => {
    ...
    console.log(`Placed ${newStone.color} at ${newStone.position.x},${newStone.position.y}. Captured: ${capturedStones.length} stones.`);
    ...
    if (capturedStones.length > 0) {
      ...
      // Play capture sound with a slight delay to separate it from placement sound
      // This is important because playing two sounds simultaneously can cause issues on some devices/browsers
      setTimeout(() => {
        console.log(`Triggering capture sound for ${newStone.color}'s move`);
        playCaptureSound();
      }, 200);
    }
