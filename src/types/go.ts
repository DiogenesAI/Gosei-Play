...
export interface GameState {
  ...
  // ✅ ISSUE #1 FIX: Renamed for clarity to eliminate ambiguity
  // capturedByWhite: number of BLACK stones captured BY white player
  // capturedByBlack: number of WHITE stones captured BY black player
  capturedStones: {
  ...
  lastMoveTime?: number;
  score?: {
    // Support both old format (number) and new format (object with total)
    black: number | { total: number; territory: number; prisoners: number; stones?: number; captures?: number; deadStones?: number };
    white: number | { total: number; territory: number; prisoners: number; stones?: number; captures?: number; deadStones?: number; komi: number };
