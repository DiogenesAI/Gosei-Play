...
import { GameState, Player, GameMove, Position, StoneColor, Stone } from '../../types/go';
...
const GameInfo: React.FC<GameInfoProps> = ({
  ...
  return (
    <div className={`game-info bg-white text-neutral-900 p-3 sm:p-4 rounded-lg shadow-lg border border-neutral-200 ${isTablet
      ? 'w-[600px] mx-auto'
      : isMobile
        ? 'w-full'
      ...
      <div className={`grid grid-cols-2 gap-2 ${isTablet ? 'gap-6' : 'sm:gap-4'} mb-3 sm:mb-4`}>
        {/* Black Player */}
        <div className={`player-card p-2 ${isTablet ? 'p-6' : 'sm:p-5'} rounded-lg transition-all duration-200 ${currentTurn === 'black' ? 'bg-neutral-100 ring-2 ring-blue-500' : 'bg-neutral-50'
          }`}>
          <div className="flex flex-col items-center">
            ...
            <div className="text-center mt-2 sm:mt-4">
              ...
              <div className={`${isTablet ? 'text-base' : 'text-xs sm:text-base'} text-neutral-700 mt-0.5 sm:mt-1.5 font-medium bg-neutral-200 px-2 sm:px-3 py-0.5 sm:py-1 rounded-md`}>
                Captured: {capturedStones?.capturedByBlack || 0}
              </div>
            </div>
          </div>
        </div>
        ...
        {/* White Player */}
        <div className={`player-card p-2 ${isTablet ? 'p-6' : 'sm:p-5'} rounded-lg transition-all duration-200 ${currentTurn === 'white' ? 'bg-neutral-100 ring-2 ring-blue-500' : 'bg-neutral-50'
          }`}>
          <div className="flex flex-col items-center">
            {/* Player Avatar */}
            <PlayerAvatar
            ...
            <div className="text-center mt-2 sm:mt-4">
              ...
              <div className={`${isTablet ? 'text-base' : 'text-xs sm:text-base'} text-neutral-700 mt-0.5 sm:mt-1.5 font-medium bg-neutral-200 px-2 sm:px-3 py-0.5 sm:py-1 rounded-md`}>
                Captured: {capturedStones?.capturedByWhite || 0}
              </div>
            </div>
          </div>
        </div>
      ...
      <div className={`text-center p-2.5 mb-4 rounded-lg bg-neutral-50 border border-neutral-200 ${isTablet ? 'text-lg p-4' : ''
        }`}>
        {status === 'playing' ? (
          <div className="flex items-center justify-center gap-2.5">
            <div className={`w-3.5 h-3.5 rounded-full ${currentTurn === 'black'
      ...
      {/* Undo Request Status */}
      {undoRequest && (
        <div className={`text-center p-2 mb-4 rounded-lg ${isDarkMode
          ? 'bg-yellow-900/20 border border-yellow-800/50'
          : 'bg-yellow-50 border border-yellow-200'
          } ${isTablet ? 'text-base p-3' : 'text-sm'}`}>
          <div className="flex items-center justify-center gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 ${isDarkMode ? 'text-yellow-400' : 'text-yellow-600'
      ...
      <div className={`space-y-2 ${isTablet ? 'space-y-4' : 'sm:space-y-3'} mb-3 sm:mb-4`}>
        ...
        <div className={`grid grid-cols-2 gap-2 ${isTablet ? 'gap-4' : 'sm:gap-3'}`}>
          <button
            ...
            className={`flex items-center justify-center gap-1 ${isTablet
          ...
          <button
            ...
            className={`flex items-center justify-center gap-2 ${isTablet
        ...
        {shouldShowForceScoring && (
          <div className="w-full">
            <button
              ...
              className={`w-full flex items-center justify-center gap-2 ${isTablet
                ? 'text-base gap-4 px-6 py-4'
                : 'sm:gap-2 px-4 py-2.5'
                } ${isDarkMode
      ...
      <div className={`grid grid-cols-2 gap-2 ${isTablet ? 'gap-4' : 'sm:gap-3'} mt-3 sm:mt-4`}>
        ...
        {/* Settings */}
        <div className={`p-2 ${isTablet ? 'p-4' : 'sm:p-3'} rounded-md ${isDarkMode ? 'bg-neutral-800/80' : 'bg-neutral-50'
          }`}>
          <h3 className={`${isTablet ? 'text-lg' : 'text-sm sm:text-base'} font-semibold mb-1 sm:mb-2 ${isDarkMode ? 'text-white' : 'text-neutral-800'
            }`}>
            Settings
          </h3>
          <div className={`space-y-1 ${isTablet ? 'space-y-2' : 'sm:space-y-2'}`}>
            ...
            <div className={`flex items-center justify-between text-xs ${isTablet ? 'text-base' : ''}`}>
              ...
              <button
                ...
                className={`px-1.5 ${isTablet ? 'px-2' : ''} py-0.5 ${isTablet ? 'py-1' : ''} rounded text-xs ${autoSaveEnabled
      ...
      {isSpectator && gameState.history.length > 0 && status !== 'finished' && (
        <div className="mt-3 sm:mt-4">
          <div className={`p-3 rounded-lg border ${isDarkMode
            ...
            <div className={`font-semibold mb-2 ${isDarkMode ? 'text-neutral-200' : 'text-neutral-800'
              }`}>
              Review Game
            </div>
            <div className="flex flex-wrap gap-1 mb-2 justify-center">
              <button
                className={`btn btn-xs px-2 py-1 text-xs ${isDarkMode
              ...
              <span className={`px-2 py-1 text-xs font-medium ${isDarkMode ? 'text-neutral-300' : 'text-neutral-600'
                }`}>
            ...
            {isReviewing && (
              <div className="text-center">
                <button
                  className={`btn btn-xs px-3 py-1 text-xs ${isDarkMode
      ...
      {(status === 'scoring' || status === 'finished') && (
        <div className={`p-4 rounded-lg mt-4 border ${isTablet ? 'text-base' : 'text-sm'} ${isDarkMode
          ? 'bg-neutral-800/80 border-neutral-700'
          : 'bg-neutral-50 border-neutral-200'
          }`}>
          <h3 className={`${isTablet ? 'text-base' : 'text-sm sm:text-base'} font-semibold mb-3 ${isDarkMode ? 'text-white' : 'text-neutral-800'
            }`}>
            Score Breakdown
          </h3>
            ...
            <div className={`grid grid-cols-3 gap-2 ${isTablet ? 'text-base' : 'text-sm'}`}>
              ...
              {(gameState.scoringRule === 'japanese' || gameState.scoringRule === 'aga' || gameState.scoringRule === 'ing') && (
                <>
                  ...
                  <div className={`text-center ${isDarkMode ? 'text-white' : 'text-neutral-900'}`}>{capturedStones.capturedByWhite.toFixed(1)}</div>
                  <div className={`text-center ${isDarkMode ? 'text-white' : 'text-neutral-900'}`}>{capturedStones.capturedByBlack.toFixed(1)}</div>
                </>
              )}
              ...
              <div className={`text-center font-bold text-base ${isDarkMode ? 'text-white' : 'text-neutral-900'}`}>{((score.blackTerritory ?? 0) - capturedStones.capturedByWhite - deadStonesByColor.black).toFixed(1)}</div>
              <div className={`text-center font-bold text-base ${isDarkMode ? 'text-white' : 'text-neutral-900'}`}>{((score.whiteTerritory ?? 0) - capturedStones.capturedByBlack - deadStonesByColor.white + (score.komi || gameState.komi)).toFixed(1)}</div>
            </div>
          ) : (
            <div className={`text-center p-3 ${isTablet ? 'text-base' : 'text-sm'} ${isDarkMode ? 'text-white' : 'text-neutral-900'
              }`}>
          ...
          {status === 'finished' && score && (
            <div className={`mt-4 text-center ${isTablet ? 'text-base' : 'text-sm'}`}>
              <div className={`inline-block px-4 py-2 rounded-lg font-semibold ${gameState.winner === 'black'
                ? 'bg-black text-white'
                ...
                {gameState.winner === 'black'
                  ? 'Black wins by ' + ((typeof score.black === 'number' ? score.black : (score.black as any)?.total || 0) - (typeof score.white === 'number' ? score.white : (score.white as any)?.total || 0)).toFixed(1) + ' points'
                  : gameState.winner === 'white'
                    ? 'White wins by ' + ((typeof score.white === 'number' ? score.white : (score.white as any)?.total || 0) - (typeof score.black === 'number' ? score.black : (score.black as any)?.total || 0)).toFixed(1) + ' points'
          ...
          {status === 'scoring' && gameState.scoreConfirmation && (
            <div className={`mt-4 p-3 rounded-md border ${isDarkMode ? 'bg-neutral-700/50 border-neutral-600' : 'bg-blue-50 border-blue-200'
              }`}>
              <h4 className={`text-sm font-medium mb-2 ${isDarkMode ? 'text-white' : 'text-neutral-800'
                }`}>
                Score Confirmation Status
              </h4>
              <div className="flex items-center justify-between space-x-4">
                <div className="flex items-center space-x-2">
                  <div className={`w-3 h-3 rounded-full ${gameState.scoreConfirmation.black
                    ? 'bg-green-500'
                    : 'bg-neutral-400'
                    }`}></div>
                ...
                <div className="flex items-center space-x-2">
                  <div className={`w-3 h-3 rounded-full ${gameState.scoreConfirmation.white
                    ? 'bg-green-500'
                    : 'bg-neutral-400'
                    }`}></div>
                  <span className={`text-sm ${isDarkMode ? 'text-neutral-300' : 'text-neutral-600'
              ...
              {gameState.scoreConfirmation.black && gameState.scoreConfirmation.white && (
                <div className={`mt-2 text-center text-sm font-medium ${isDarkMode ? 'text-green-400' : 'text-green-600'
                  }`}>
          ...
          {status === 'scoring' && isSpectator && (
            <div className={`mt-4 p-3 rounded-md text-center ${isDarkMode
