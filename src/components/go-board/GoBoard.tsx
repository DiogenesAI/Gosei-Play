...
const GoBoard: React.FC<GoBoardProps> = ({
  ...
  const getStoneAtPosition = useCallback((x: number, y: number): Stone | undefined => {
  ...
  const isDeadStone = useCallback((x: number, y: number): boolean => {
    return deadStones.some(stone => stone.x === x && stone.y === y);
  }, [deadStones]);

  // Check if a position is part of a territory and get its owner
  const getTerritoryOwner = useCallback((x: number, y: number): StoneColor | null => {
  ...
  const isValidHandicapPoint = useCallback((x: number, y: number): boolean => {
    if (!isHandicapPlacement) return false;
    return isHandicapPoint({ x, y }, board.size) && !getStoneAtPosition(x, y);
  }, [isHandicapPlacement, board.size, getStoneAtPosition]);
  ...
  const handleMouseOver = useCallback((x: number, y: number) => {
    if (!isReviewing && !isScoring && isPlayerTurn && isValidPlacement(x, y)) {
      setHoverPosition({ x, y });
    }
  }, [isReviewing, isScoring, isPlayerTurn, isValidPlacement]);
  ...
  }, [isReviewing, isPlayerTurn, isValidPlacement, isScoring, onPreviewPositionChange]);
  ...
  }, [isReviewing, stonesToDisplay, cellSize, lastMove, isDeadStone, themeConfig]);
  ...
  const renderCellOverlays = useCallback(() => {
    ...
    return overlays;
      ...
      handleTouchStart, isPlayerTurn, isValidPlacement, isScoring, currentTurn, themeConfig.stoneEffects.black, 
      themeConfig.stoneEffects.white, themeConfig.stoneGradient]);
