...
const GameCompleteModal: React.FC<GameCompleteModalProps> = ({ onClose, onPlayAgain }) => {
  ...
  }, [gameState?.socket, currentPlayer, navigate, handleReturnHome]);
  ...
  }, [gameState, waitingForNewGame, handleClose]);
  ...
  if (waitingForNewGame) {
    return createPortal(
      <div
        className={`fixed inset-0 bg-black transition-opacity duration-300 z-[9999] ${visible ? 'bg-opacity-50' : 'bg-opacity-0'
          }`}
  ...
  return createPortal(
    ...
    >
      <div className={`fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white dark:bg-gray-800 rounded-xl shadow-2xl overflow-hidden w-[90%] max-w-md transition-all duration-300 ${visible ? 'scale-100' : 'scale-95'}`}>
        ...
        {/* Result Card */}
        <div className="p-6">
          <div className={`mx-auto mb-6 text-center p-5 rounded-lg shadow-md ${winner === 'black' ? 'bg-gray-900 text-white' :
              ...
              <div className="flex justify-center items-center gap-4">
                <div>
                  ...
                  <div className="text-2xl font-bold">
                    {(typeof score.black === 'number' ? score.black : (score.black as any)?.total || 0).toFixed(1)}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="text-2xl font-bold">
                    {(typeof score.white === 'number' ? score.white : (score.white as any)?.total || 0).toFixed(1)}
                  </div>
                </div>
              </div>
            ) : (
