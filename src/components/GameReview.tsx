...
const GameReview: React.FC<GameReviewProps> = ({ gameState, onBoardStateChange }) => {
  ...
  const getAdjacentPositions = useCallback((pos: Position, boardSize: number): Position[] => {
  ...
  const getGroup = useCallback((pos: Position, stones: Stone[], boardSize: number): Position[] => {
  ...
  const hasNoLiberties = useCallback((group: Position[], stones: Stone[], boardSize: number): boolean => {
  ...
  const calculateBoardState = useCallback((moveIndex: number): Stone[] => {
    let stones: Stone[] = [];
    ...
    return stones;
  }, [gameState.gameType, gameState.handicap, gameState.board.stones, gameState.history, gameState.board.size, getGroup, hasNoLiberties, getAdjacentPositions]);
  ...
  }, [currentMoveIndex, gameState, onBoardStateChange, showFinalPosition, calculateBoardState]);
  ...
  }, [currentMoveIndex, gameState.history.length, isPlaying]);
  ...
  }, [showFinalPosition, goToPreviousMove, goToNextMove, togglePlay]);
