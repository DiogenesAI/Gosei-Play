...
import { GameState, Position, Player, StoneColor, GameMove, GameOptions, GameType } from '../types/go';
...
import { getHandicapStones, getAdjustedKomi } from '../utils/handicapUtils';
import { playStoneSound, playCaptureSound } from '../utils/soundUtils';
import {
  getDeadStoneGroup,
  findStoneAt,
  getConnectedGroup,
...
export const GameProvider: React.FC<GameProviderProps> = ({
  ...
  useEffect(() => {
    ...
    if (socket) {
      ...
      socket.on('gameState', (updatedGameState: GameState) => {
        ...
        const isCurrentGame = gameState && gameState.id === updatedGameState.id;
        const hasCurrentPlayer = currentPlayer && updatedGameState.players.find(
        ...
        if (isCurrentGame || hasCurrentPlayer) {
          ...
          const updatedCurrentPlayer = currentPlayer && updatedGameState.players.find(
            p => p.id === currentPlayer.id
          );
          ...
          if (updatedCurrentPlayer && currentPlayer && currentPlayer.id !== updatedCurrentPlayer.id) {
  ...
  useEffect(() => {
    ...
    const timePerMoveFromControl = state.gameState?.timeControl?.timePerMove ?? 0;
    const timePerMoveFromGame = state.gameState?.timePerMove ?? 0;

    if (socket && gameStatus === 'playing' &&
      (timePerMoveFromControl > 0 || timePerMoveFromGame > 0)) {
      // Send timer ticks less frequently since we now have better server synchronization
      const timerInterval = setInterval(() => {
        if (gameStatus === 'playing' && socket && gameId) {
  ...
  }, [state.socket, state.gameState?.status, state.gameState?.id, state.gameState?.timeControl?.timePerMove, state.gameState?.timePerMove]);
  ...
  useEffect(() => {
    if (state.socket) {
      ...
      state.socket.on('moveMade', (moveData) => {
        ...
        if (currentPlayerId && moveData.playerId !== currentPlayerId && !moveData.pass && moveData.position) {
          playStoneSound();
        }
      });
  ...
  }, [state.socket, state.currentPlayer?.id]);
  ...
  const createGame = (options: GameOptions & { playerName?: string; captcha?: any; captchaAnswer?: any; multiCaptcha?: any; captchaAnswers?: any }) => {
    ...
    const gameState: GameState = {
      ...
      history: [],
      capturedStones: { capturedByWhite: 0, capturedByBlack: 0 },
      status: 'waiting',
      winner: null,
  ...
  const placeStone = (position: Position) => {
    ...
    // Check for captures to play sound
    const currentCaptured = currentPlayer.color === 'black'
      ? gameState.capturedStones.capturedByBlack
      : gameState.capturedStones.capturedByWhite;

    const newCaptured = currentPlayer.color === 'black'
      ? updatedGameState.capturedStones.capturedByBlack
      : updatedGameState.capturedStones.capturedByWhite;

    if (newCaptured > currentCaptured) {
      playCaptureSound();
    }
    ...
    if (historyLength >= 2) {
      ...
      if (isPassMove(lastMove) && isPassMove(secondLastMove)) {
        ...
        try {
          ...
          updatedGameState.score = {
            ...scoringUtils.score,
            blackCaptures: updatedGameState.capturedStones.capturedByWhite,
            whiteCaptures: updatedGameState.capturedStones.capturedByBlack
          };
        } catch (e) {
  ...
  const confirmScore = () => {
    ...
    console.log("Both players confirmed score - sending final confirmation to server");

    // Update game state with both confirmations
    const updatedGameState: GameState = {
      ...gameState,
      scoreConfirmation: updatedConfirmation
    };
    ...
    // Emit FINAL confirmation to server
    // The server will calculate the authoritative score and broadcast gameFinished
    if (state.socket && state.socket.connected) {
      const confirmationData = {
        gameId: gameState.id,
        playerId: currentPlayer.id,
      ...
      console.log(`Emitting final score confirmation for ${currentPlayer.color} player`);
      console.log('Server will calculate and broadcast the final score');
      state.socket.emit('confirmScore', confirmationData);
    } else {
      console.warn('Socket not connected, cannot send final confirmation');
    ...
    try {
      ...
      console.error('Failed to save game state after final confirmation:', e);
    }
  };
