...
```bash
sudo cp /etc/nginx/sites-available/gosei-svr01.ecreative.asia \
       /etc/nginx/sites-available/gosei-svr01.ecreative.asia.backup
```

#### 2. Create SSL-Enabled Configuration
```bash
sudo nano /etc/nginx/sites-available/gosei-svr01.ecreative.asia
```
...
server {
    listen 80;
    server_name gosei-svr01.ecreative.asia;
...
server {
    ...
    # SSL configuration
    ssl_certificate /etc/letsencrypt/live/gosei-svr01.ecreative.asia/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/gosei-svr01.ecreative.asia/privkey.pem;
...
# Certificate expiry
openssl x509 -in /etc/letsencrypt/live/gosei-svr01.ecreative.asia/cert.pem -noout -enddate

# Test SSL connection
curl -I https://gosei-svr01.ecreative.asia/
```

### Test Services
```bash
# Check HTTP redirect (should return 301)
curl -I http://gosei-svr01.ecreative.asia/

# Test HTTPS
curl -I https://gosei-svr01.ecreative.asia/

# Test health endpoint
curl https://gosei-svr01.ecreative.asia/health
```
...
sudo cp /etc/nginx/sites-available/gosei-svr01.ecreative.asia.backup \
       /etc/nginx/sites-available/gosei-svr01.ecreative.asia
sudo systemctl reload nginx
```
...
1. **Check certificate paths exist:**
   ```bash
   ls -la /etc/letsencrypt/live/gosei-svr01.ecreative.asia/
   ```
...
3. **Check DNS resolution:**
   ```bash
   nslookup gosei-svr01.ecreative.asia
   ```
...
1. **Test your site:** https://gosei-svr01.ecreative.asia
2. **Check SSL grade:** https://www.ssllabs.com/ssltest/analyze.html?d=gosei-svr01.ecreative.asia
